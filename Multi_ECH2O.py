...
parser.add_option("--batchID",dest="batchID",metavar="batchID",
                  help="The batch ID")

parser.add_option("--ifWaterQualitySimulation",dest="ifWaterQualitySimulation",metavar="ifWaterQualitySimulation",
                  help="if activate the water quality simulation")
...
class Config:
    PATH_MAIN = os.getcwd()+'/'
    #PATH_MAIN = '/data/scratch/wusongj/paper7/run/batch_'+options.batchID
...
# Activate water quality model?
if options.ifWaterQualitySimulation == None:
    options.ifWaterQualitySimulation == 0
Config.ifWaterQualitySimulation = int(copy.copy(options.ifWaterQualitySimulation))
...
if Config.runECH2O == 1:
    ...
    Config.output_tmp = os.path.abspath(os.path.join(Config.PATH_OUT,'Outputs_tmp'))
...
if Config.runECH2O == 1:
    ...
    Config.PATH_SPA_REF  = '/home/wusongj/dmc/forHydrology/'+'Spatial_'+Config.Resol+'m'
    Config.PATH_CLIM_REF = '/home/wusongj/dmc/forHydrology/'+'Climate'    
    ...
    if len(glob.glob(Config.output_tmp))==0:
        os.system('mkdir '+Config.output_tmp)
    else:
        os.system('rm -f '+Config.output_tmp+'/*')
    ...
    os.symlink(os.path.join('/home/wusongj/ech2o_iso_v2/Release-Linux/', exe_ech2o) , os.path.join(Config.PATH_OUT,exe_ech2o))
...
if Config.mode == 'calib_runs':
    ...
    for it in range(it0,Opti.nit):
        ...
        if ECH2O_Tools.runOK(Data, Opti, Config) == 0:
            ...
            if ECH2O_Tools.runOK(Data, Opti, Config) == 0:
                print("both attempts failed, "+Config.PATH_EXEC+"BasinSummary.txt is missing/empty...")
        ...
        if ECH2O_Tools.runOK(Data, Opti, Config) == 1:
            ...
            # Then run the Nitrate module!
            if Config.ifWaterQualitySimulation==1:
                ECH2O_Tools.run_NO3Module(Config.mode)
        ...
        open(Config.PATH_OUT+'/done1', "w").close()
        os.chdir(Config.PATH_OUT)
        #clean up
        if os.path.exists(Config.PATH_EXEC):
            shutil.rmtree(Config.PATH_EXEC)
        os.mkdir(Config.PATH_EXEC)

        
        #os.system('rm -f '+Config.PATH_EXEC+'/*')
    open(Config.PATH_OUT+'/done', "w").close()
...
if Config.mode == 'forward_runs':
    ...
    for it in range(int(options.nEns)):
        ...
        try:
            ...
            start = time.time()
            os.system(Config.cmde_ech2o+' > '+Config.PATH_EXEC+'/ech2o.log')  
            #os.system(Config.cmde_ech2o) # todo
            #print('    run time:',time.time() - start,'seconds (limit at '+Config.tlimit+')')
            # Check if it ran properly
            os.chdir(Config.PATH_EXEC)       
                ...
                if ECH2O_Tools.runOK(Data, Opti, Config) == 0:
                    ...
                    if len(glob.glob(f_failpar))==0:
                        ...
                        with open(f_failpar,'a') as f_in:
                            f_in.write(str(it+1)+','+','.join([str(x) for x in Opti.x])+'\n')
                        os.system('rm ' +Config.PATH_OUT+'/core*')
                    os.system('mv '+Config.PATH_EXEC+'/ech2o.log '+Config.PATH_OUT+'/ech2o.log')
                    # If it is the very first iteration, record it for mana_outputs later
                    if it==0:
                        Opti.begfail = 1
            ...
            if ECH2O_Tools.runOK(Data, Opti, Config) == 1:
                ...
                # todo only for external coupling of no3 simulation
                try:
                    if os.path.exists(Config.PATH_OUT + '/tmp_tmp'):
                        shutil.rmtree(Config.PATH_OUT + '/tmp_tmp')               
                    shutil.copytree(Config.PATH_EXEC, Config.PATH_OUT + '/tmp_tmp')
            ...
            print('This is the error message from forward run of Ech2o-iso    :', e)
