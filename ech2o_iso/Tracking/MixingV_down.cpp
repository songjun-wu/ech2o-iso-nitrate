			    ...
			    int step) // 0 for SolveSurface, 1 for GWrouting
{
  ...
  // Groundwater ------------------------------------------------------------------------
	...
	/*
  // added by Songjun, 2024.11.15
  if(ctrl.sw_2H)  
  	_d2HExtraGWtoLat->matrix[r][c] = InOutMix(bsn.getExtraGW()->matrix[r][c], _d2HExtraGWtoLat->matrix[r][c],
					   leak, _d2Hgroundwater->matrix[r][c], 0, mixmod);
  if(ctrl.sw_18O)
	_d18OExtraGWtoLat->matrix[r][c] = InOutMix(bsn.getExtraGW()->matrix[r][c], _d18OExtraGWtoLat->matrix[r][c],
					   leak, _d18Ogroundwater->matrix[r][c], 0, mixmod);
  if(ctrl.sw_Age)
   _AgeExtraGWtoLat->matrix[r][c] = InOutMix(bsn.getExtraGW()->matrix[r][c], _AgeExtraGWtoLat->matrix[r][c],
					   leak, _Agegroundwater->matrix[r][c], 0, mixmod);
  */
